...
describe '通常ユーザが自分の作ったアイデアを削除する' do
...
describe 'スーパーユーザがアイデアを削除する' do
  ...
  context 'ログインしてトップページに遷移したとき' do
    ...
    it '全てのアイデアに削除ボタンが表示されていること' do
      expect(find("#idea_#{user_idea.id}")).to have_link '削除'
      expect(find("#idea_#{other_user_idea.id}")).to have_link '削除'
    end

    context '自分以外のユーザが作ったアイデアの削除ボタンを押して、ダイアログで"OKをクリックしたとき"', js: true do
      before do
        find("#idea_#{other_user_idea.id}").click_link '削除'
      end

      it '対象のアイデアが削除されていること' do
        expect(page).to have_css "#idea_#{user_idea.id}"
        expect(page).to have_no_css "#idea_#{other_user_idea.id}"
      end
    end
  end
end
